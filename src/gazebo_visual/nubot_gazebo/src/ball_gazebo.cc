...
void BallGazebo::detect_ball_out(void)
{
    ...
    if(fabs(pos_x)>field_length_/2.0)
    {
        ...
        ignition::math::Pose3d  target_pose( ignition::math::Vector3d (a*(field_length_/2.0-0.02), pos_y, 0.12), ignition::math::Quaterniond::Zero );
        football_model_->SetWorldPose(target_pose);
        football_model_->SetLinearVel(ignition::math::Vector3d::Zero);
    }
    else if(fabs(pos_y) > field_width_/2.0)
    {
        ignition::math::Pose3d  target_pose( ignition::math::Vector3d (pos_x, b*(field_width_/2.0 - 0.02), 0.12), ignition::math::Quaterniond::Zero);
        football_model_->SetWorldPose(target_pose);
        football_model_->SetLinearVel(ignition::math::Vector3d::Zero);
    }
}
