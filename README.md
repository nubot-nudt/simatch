...
- robot_code: 机器人的感知、规划、运动控制等方面，由NuBot队伍的机器人代码改造而成,其中主要的策略等(在nubot_control软件包中)部分已经删除，除nubot_hwcontroller以外，选手应自行编写，其余部分可以直接沿用，也可随意改动。
...
6. **增加机器人罚下/上场功能，由裁判裁定后，通过自动裁判盒进行判罚，罚下/上场必须在死球期间进行**
下场/上场流程：
```
	(1)暂停比赛 : 按下空格进入人工操作模式。
	(2)罚下/上场: 数字键2-5对应cyan 2-5号机器人，6-9对应magenta 2-5号机器人。
	(3)开球     : 按照规则，发布对应开球命令，如kick-off,free-kick,corner-kick。
	(4)继续比赛 : 按下空格，退出人工操作模式。
```
